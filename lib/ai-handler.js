...
export function markKeyRateLimited(key, model, errorMessage) {
    ...
    // Parse "Please try again in 43m14.592s" or "Please retry in 54s"
    const match = errorMessage.match(/(?:try again|retry) in (?:(\d+)h)?(.*?)?(?:(\d+)m)?(.*?)?(?:([\d.]+)s)?/i);
    if (match) {
        // More robust extraction just in case
        const timeStr = errorMessage.match(/(?:try again|retry) in (.*?s)/i)?.[1] || "";
        let totalSeconds = 0;
...
/**
 * Returns an array of all Groq API keys found in process.env.
 *
 * Automatically discovers EVERY env var whose name starts with "GROQ_API_KEY"
 * (case-insensitive). Add as many as you want to .env — they'll be picked up:
 *
 *   GROQ_API_KEY=...
 *   GROQ_API_KEY_1=...
 *   GROQ_API_KEY_2=...
 *   GROQ_API_KEY_SUB_1=...
 *   GROQ_API_KEY_REGULAR=...
 *   ...any suffix works
 */
export function getGroqChain() {
    const pattern = /^GROQ_API_KEY/i;
    const seen = new Set();
    const keys = [];
...
/**
 * Returns an array of all Gemini API keys found in process.env.
 * Automatically discovers EVERY env var whose name starts with "GEMINI_API_KEY"
 * (case-insensitive).
 */
export function getGeminiChain() {
    const pattern = /^GEMINI_API_KEY/i;
    const seen = new Set();
    const keys = [];
