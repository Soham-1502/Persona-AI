...
export function markKeyRateLimited(key, model, errorMessage) {
    ...
    const match = errorMessage.match(/try again in (?:(\d+)h)?(.*?)?(?:(\d+)m)?(.*?)?(?:([\d.]+)s)?/);
    if (match) {
        // More robust extraction just in case
        const timeStr = errorMessage.match(/try again in (.*?s)/)?.[1] || "";
        let totalSeconds = 0;
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            try {
                ...
                if (isRateLimit) {
                    ...
                    console.warn(`⚠️ ${errorMsg} -> Auth Error. Skipping key...`);
                    markKeyRateLimited(key, model, "try again in 24h"); // Skip invalid keys completely
                    continue;
